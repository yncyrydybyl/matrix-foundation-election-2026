...
24. *"is it deployed?"* — verified Pages had built but discovered the og:* tags were inside the bundler template (only present after JavaScript runs), invisible to social-link crawlers. Duplicated the og:* / twitter:* / description tags into the static outer `<head>` so Element et al. can read them on first byte.
25. *"please fix the preview of the post it looks horrible — don't use text as a logo. use the logo from matrix.org/branding/"* — replaced the hand-drawn `[matrix]` wordmark in `og-image.png` with the official PNG from the Matrix Foundation branding kit (`matrix-logos.zip`), composited onto the same dark plate.
26. *"the logos are still fonts and not the graphic, right?"* — yes; the in-page masthead `[matrix]` mark was still SVG `<text>` in Helvetica Neue. Bundled the official SVG (`matrix-logo.svg`) into the repo and switched the `MatrixMark` component's `official` style to render it as `<img>` inside the dark plate, inverted to white. No more text-as-logo anywhere on the page.
27. *"no logo"* — followed up with a screenshot of the masthead showing the dark plate but no logo inside. Root cause: `MatrixMark` was being called with `size={28}` and the plate's hard-coded `padding: 14px 28px` consumed the entire 28 px height, leaving the `<img>` at zero height. Switched to proportional padding (45 % vertical, 85 % horizontal) so the logo always has room to breathe.
28. *"and what about the logo which is shown for a moment when loading first time? that should also be the logo and not a font"* — replaced the inline `__bundler_thumbnail` SVG (Helvetica `<text>` + hand-drawn brackets) in the outer `<head>` with an `<img src="matrix-logo.svg">` rendered white via the same invert filter, so the brand asset is correct even during the bundler's initial unpack splash.
